...
class GaussianMultinomialDiffusion(torch.nn.Module):
    def __init__(
        ...
        print(self.num_classes_expanded.shape)

        self.slices_for_classes = [torch.arange(self.num_classes[0])]
        offsets = np.cumsum(self.num_classes)
        for i in range(1, len(offsets)):
            self.slices_for_classes.append(torch.arange(offsets[i - 1], offsets[i]))
        ...
        self.gen_timesteps = gen_timesteps
    ...
    def gaussian_p_mean_variance(
        ...
        if clip_denoised:
            pred_xstart = clip(pred_xstart, -1, 1)
    ...
    def gaussian_p_sample(
        ...
        device = x.device
        noise = sample_noise(x.shape[0], dmss)
    ...
    def multinomial_kl(self, log_prob1, log_prob2, mask):
        kl = (log_prob1.exp() * (log_prob1 - log_prob2)) * mask
        return kl.sum(dim=1)
    ...
    def log_sample_categorical(self, logits):
        ...
        for i in range(len(self.num_classes)):
            sl = self.slices_for_classes[i]
            one_class_logits = logits[:, sl[0]:sl[-1]]
    ...
    def kl_prior(self, log_x_start, mask):
        b = log_x_start.size(0)
        device = log_x_start.device
        ...
        kl_prior = self.multinomial_kl(log_qxT_prob, log_half_prob, mask)
        return sum_except_batch(kl_prior)

    def compute_Lt(self, model_out, log_x_start, log_x_t, t, out_dict, mask, detach_mean=False):
        ...
        decoder_nll = -log_categorical(log_x_start, log_model_prob, mask)
    ...
    def _multinomial_loss(self, model_out, log_x_start, log_x_t, mask, t, pt, out_dict):

        if self.multinomial_loss_type == 'vb_stochastic':
            kl = self.compute_Lt(
                model_out, log_x_start, log_x_t, t, out_dict, mask
            )
            kl_prior = self.kl_prior(log_x_start, mask)
    ...
    def mixed_loss(self, cond, x, out_dict, mask, dmss):
        b = x.shape[0]
        device = x.device
        ...
        # Mask Gaussian
        model_out_num *= mask[:, :self.num_numerical_features].float()
        x_num *= mask[:, :self.num_numerical_features].float()

        # Mask Categorical
        # cat_mask = mask[:, self.num_numerical_features:].unsqueeze(-1).repeat(1,1,2).reshape(b,-1)
        # print(cat_mask)
        cat_mask = torch.concatenate(
            [
                mask[:, i].unsqueeze(-1).repeat(1,1,r).reshape(b,-1)
                for i,r in enumerate(self.num_classes)
            ],
            1
        )
        ...
        if x_cat.shape[1] > 0:
            loss_multi = self._multinomial_loss(model_out_cat, log_x_cat, log_x_cat_t, cat_mask, t, pt, out_dict) / len(self.num_classes)
    ...
    @torch.no_grad()
    def mixed_elbo(self, cond, x0, out_dict):
        b = x0.size(0)
        device = x0.device
    ...
    @torch.no_grad()
    def sample(self, cond, num_samples, y, dmss, clip_sample=False):
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.num_numerical_features), device=device)
        z_norm = sample_noise(b, dmss)
        z_norm = torch.tensor(z_norm).to(device).float()
        ...
        x_in = torch.cat([z_norm, log_z], dim=1).float()

        # Initial values before diffusion
        x_init = ohe_to_categories(log_z, self.num_classes)
        x_init = torch.cat([z_norm, x_init], dim=1).float()
        ...
        for i in reversed(range(0, self.gen_timesteps)):
            ...
            model_out_num = model_out[:, :self.num_numerical_features]
            model_out_cat = model_out[:, self.num_numerical_features:]
            z_norm = self.gaussian_p_sample(model_out_num, z_norm, t, dmss, clip_denoised=clip_sample)['sample']
            if has_cat:
